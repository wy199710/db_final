        ...
        if (msg.answer != null) {
            for (const cur of msg.answer) {
                $("#card-parent").html((index, cxt) => {
                    return cxt +
                        '<div className="card" style="position: relative;top:50px;left:200px;background-color:white;height:fit-content;width: 80%;border-radius: 10px">' +
                        '   <div class="card-body">\n' +
                        ...
                        '                <p class="card-text" style="margin-left:20px;padding-bottom: 20px;" id = "a_date">' + "Answer date： " + cur.a_date + '</p>\n' +
                        '            </div></div>';
                });
            }
        }
